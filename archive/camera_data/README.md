# camera_data.json archive

This directory preserves every superseded `camera_data.json` file before a new active file replaces it.

## Rules
- `camera_data.json` is the active file.
- Before the update job writes a new `camera_data.json`, it must archive the current active file here.
- `manifest.json` records the catalog history, SHA-256 hash, and validity window metadata for each archived file.
- Filenames use UTC archive timestamps: `camera_data__through_YYYY-MM-DDTHH-MM-SSZ.json`.
